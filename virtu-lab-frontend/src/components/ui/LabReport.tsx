...
const AIMS: Record<string, string> = {
    ...
    'microscope': 'To study cell structure by focusing and observing specimens under a compound microscope at various magnifications.',
    'cell-structure': 'To identify and label the key organelles of animal and plant cells and understand their functions.',
    'mitosis': 'To observe and identify the stages of mitotic cell division (prophase, metaphase, anaphase, telophase).',
    'anatomy': 'To study the human organ systems and understand the spatial relationships between major organs.',
};
...
const TABLE_HEADERS: Record<string, { key: string; label: string; unit: string }[]> = {
    ...
    'microscope': [
        { key: 'magnification', label: 'Magnification', unit: '×' },
        { key: 'fieldOfView', label: 'Field of View', unit: 'mm' },
        { key: 'resolution', label: 'Resolution', unit: 'μm' },
    ],
    'cell-structure': [
        { key: 'zoom', label: 'Zoom Level', unit: '×' },
        { key: 'organelle', label: 'Organelle', unit: '' },
        { key: 'function', label: 'Function', unit: '' },
    ],
    'mitosis': [
    ...
    'anatomy': [
        ...
        { key: 'organ', label: 'Organ', unit: '' },
        { key: 'system', label: 'System', unit: '' },
    ],
};
...
const TEMPLATE_RESULTS: Record<string, string> = {
    ...
    'microscope': 'At low magnification (4×), a wide field of view was observed with low detail. As magnification increased to 10× and 40×, cellular details became more visible but the field of view decreased proportionally. At 100× (oil immersion), individual organelles could be distinguished. Proper focusing with coarse and fine adjustment knobs was essential for clear images.',
    'cell-structure': 'The key organelles of eukaryotic cells were identified: the nucleus (containing DNA), mitochondria (energy production via cellular respiration), endoplasmic reticulum (protein and lipid synthesis), Golgi apparatus (packaging and secretion), and ribosomes (protein synthesis). Plant cells additionally showed cell wall, chloroplasts, and a large central vacuole.',
    ...
    'anatomy': 'The spatial arrangement of human organ systems was explored. The cardiovascular system (heart, blood vessels) occupies the thoracic cavity. The respiratory system (lungs, trachea) flanks the heart. The digestive system extends from the oral cavity through the abdominal cavity. The nervous system (brain, spinal cord) runs along the dorsal axis. These systems interconnect functionally.',
};
...
const VIVA_QUESTIONS: Record<string, string[]> = {
    ...
    'microscope': [
        'What is the difference between magnification and resolution? Why is resolution more important in microscopy?',
        'Calculate the total magnification when using a 10× eyepiece and a 40× objective lens.',
        'Explain why oil immersion is necessary for 100× objective lenses. What property of the oil is important?',
    ],
    'cell-structure': [
        'Compare and contrast the structure of a plant cell and an animal cell. List at least 4 differences.',
        'Explain the endosymbiotic theory for the origin of mitochondria and chloroplasts. What evidence supports it?',
        'Describe the structure and function of the rough endoplasmic reticulum. How is it different from smooth ER?',
    ],
    'mitosis': [
    ...
    'anatomy': [
        'Describe the double circulation system in humans. Why is it more efficient than single circulation?',
        'Explain the structure of a nephron and describe the process of urine formation (filtration, reabsorption, secretion).',
        'What is the role of the sinoatrial (SA) node in the cardiac cycle? Why is it called the natural pacemaker?',
    ],
};
...
function computeDerived(lab: LabType, row: Record<string, number>): Record<string, number> {
    ...
    switch (lab) {
        ...
        case 'microscope': {
            const mag = row.magnification ?? 10;
            out.magnification = mag;
            out.fieldOfView = parseFloat((20 / mag).toFixed(2));
            out.resolution = parseFloat((200 / mag).toFixed(1));
            break;
        }
        case 'cell-structure': {
            const z = row.zoom ?? 1;
            const oIdx = Math.min(Math.floor(z) - 1, ORGANELLE_NAMES.length - 1);
            out.zoom = z;
            out.organelle = Math.max(0, oIdx);
            out.function = Math.max(0, oIdx);
            break;
        }
        case 'mitosis': {
        ...
        case 'anatomy': {
            const rot = row.rotate ?? 0;
            const orgIdx = Math.min(Math.floor(Math.abs(rot) / 52), ORGAN_NAMES.length - 1);
            out.rotate = rot;
            out.organ = orgIdx;
            out.system = orgIdx;
            break;
...
const PERIODIC_ELEMENTS = ['H', 'He', 'Li', 'Be', 'B', 'C', 'N', 'O', 'F', 'Ne', 'Na', 'Mg', 'Al', 'Si', 'P', 'S', 'Cl', 'Ar'];
const PERIODIC_MASS = [1.008, 4.003, 6.941, 9.012, 10.81, 12.01, 14.01, 16.00, 19.00, 20.18, 22.99, 24.31, 26.98, 28.09, 30.97, 32.07, 35.45, 39.95];
const PERIODIC_EN = [2.20, 0, 0.98, 1.57, 2.04, 2.55, 3.04, 3.44, 3.98, 0, 0.93, 1.31, 1.61, 1.90, 2.19, 2.58, 3.16, 0];
const ORGANELLE_NAMES = ['Nucleus', 'Mitochondria', 'ER', 'Golgi', 'Ribosome', 'Lysosome', 'Cell Membrane'];
const ORGANELLE_FN = ['DNA storage', 'Energy (ATP)', 'Protein synthesis', 'Packaging', 'Translation', 'Digestion', 'Selective barrier'];
...
const ORGAN_NAMES = ['Heart', 'Lungs', 'Brain', 'Liver', 'Kidneys', 'Stomach', 'Intestines'];
const ORGAN_SYSTEMS = ['Cardiovascular', 'Respiratory', 'Nervous', 'Digestive', 'Excretory', 'Digestive', 'Digestive'];
...
export const LabReport: React.FC = () => {
    ...
    const result = TEMPLATE_RESULTS[activeLab] ?? 'The observations are consistent with the expected theoretical values, confirming the underlying scientific principles.';
    ...
    const rows = useMemo(() => {
        if (observations.length === 0) {
            return [computeDerived(activeLab, useLabStore.getState().inputs)];
        }
        return observations.map(obs => computeDerived(activeLab, obs));
    }, [observations, activeLab]);
                ...
                >
                    ...
                    <div className="relative z-10 px-8 py-5 border-t border-white/[0.06] bg-[#0c0c1e]/80 backdrop-blur-sm flex items-center gap-3">
                        ...
                        <motion.button
                            ...
                            className="flex-1 h-11 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white/60 hover:text-white hover:bg-white/[0.08] text-xs font-bold uppercase tracking-widest transition-all"
                        >
                            Try Again
                        </motion.button>
